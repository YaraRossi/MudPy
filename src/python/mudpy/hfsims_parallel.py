...
#            smga_center_lat=-30.58
            ...
            if Qmethod == 'no_moho':
                ...
                # get all paths arriving within the total_duration of the timeseries
                allpaths = velmod.get_ray_paths(zs, dist_in_degs)
                selected_paths = [p for p in allpaths if p.time <= total_duration]

                # get all S arriving paths
                allS = [p for p in selected_paths if p.name.lower().endswith('s')]
            ...
            if Pwave==True:
                ...
                for directP in P_paths:
                    ...
                    #Build the entire path term
                    G_P=(I_P*Q_P)/path_length_P # I changed this back, since I don't know why one would put it to one and also in stochastic_simulation its like this.
                    #G_P=(1*Q_P)/path_length_P
                ...
                for directS in S_paths:
                    ...
                    if window_type == 'saragoni_hart':
                        ...
                        w_p,w_s=hfsims.windowed_gaussian(3*duration,hf_dt,window_type='cua',M=Mw,dist_in_km=dist/1000,
                                                         Pcoeff=Pcoeff,Scoeff=Scoeff)
                        w = w_s-w_s.mean() #remove any DC component
