...
def windowed_gaussian(duration,hf_dt,window_type='saragoni_hart',M=5.0,dist_in_km=50.,std=1.0,ptime=10,stime=20,Pcoeff=0,Scoeff=12):
    ...
    if window_type=='saragoni_hart':
        ...
        ptime=0
        stime=0
        window_p,window_s=cua_envelope(M,dist_in_km,t,ptime,stime,Pcoeff=Pcoeff,Scoeff=Scoeff)
...
def apply_spectrum(w,A,f,hf_dt,is_gnss=False,gnss_scale=1/2**0.5, is_cua = False):
    '''
...
def cua_envelope(M,dist_in_km,times,ptime,stime,Pcoeff=0,Scoeff=12):
    ...
    # coefficienstime
    t_rise_p  = 10**(alpha_t_rise[Pcoeff] * M + beta_t_rise[Pcoeff] * dist_in_km + delta_t_rise[Pcoeff] * log10(dist_in_km) + mu_t_rise[Pcoeff])*0.25
    t_rise_s  = 10**(alpha_t_rise[Scoeff] * M + beta_t_rise[Scoeff] * dist_in_km + delta_t_rise[Scoeff] * log10(dist_in_km) + mu_t_rise[Scoeff])*0.25
    delta_t_p = 10**(alpha_delta_t[Pcoeff] * M + beta_delta_t[Pcoeff] * dist_in_km + delta_delta_t[Pcoeff] * log10(dist_in_km) + mu_delta_t[Pcoeff])*0.25
    delta_t_s = 10**(alpha_delta_t[Scoeff] * M + beta_delta_t[Scoeff] * dist_in_km + delta_delta_t[Scoeff] * log10(dist_in_km) + mu_delta_t[Scoeff])*0.25
